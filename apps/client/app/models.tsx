...
export default function ModelsScreen() {
  ...
  useEffect(() => { if (target.ready) void refresh(); }, [target.ready, target.activeBaseUrl]);

  return (
    <Screen title="Models & Engines" subtitle={`Catalog from ${target.mode}: ${target.activeBaseUrl}`} action={<ActionButton label="Refresh" onPress={() => void refresh()} />}>
      {models.map((model) => (
        <Card key={model.model_id} title={model.name || model.model_id} subtitle={`${model.capability}${model.provider ? ` · ${model.provider}` : ''}`}>
          <Text style={{ color: colors.muted }}>{model.model_id}</Text>
          <Text style={{ color: colors.muted }}>Install state: {model.installation_status ?? 'unknown'}</Text>
