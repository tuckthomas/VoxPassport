...
export default function VoiceProfilesScreen() {
  ...
  async function refresh() {
    setError('');
    try { setProfiles((await api.voiceProfiles()).profiles); }
  ...
  return (
    <Screen title="Voice Profiles" subtitle="Voice identity stays separate from the selected TTS/provider implementation." action={<ActionButton label="Refresh" onPress={() => void refresh()} />}>
      {profiles.map((profile) => (
        <Card key={profile.profile_id} title={profile.profile_name} subtitle={profile.is_active ? 'Active profile' : profile.status}>
