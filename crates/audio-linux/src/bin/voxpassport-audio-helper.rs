...
use livetranslator_audio_linux::LinuxAudioPlatform;
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let platform = LinuxAudioPlatform::new();
    match command.as_str() {
        "probe" => {
            ...
            let virtual_microphone_output =
                LinuxAudioPlatform::has_voxpassport_virtual_microphone(&endpoints);
            println!(
                "{}",
        ...
        "help" | "--help" | "-h" => {
            println!("VoxPassport PipeWire audio helper");
            println!("  probe");
            println!("  devices");
