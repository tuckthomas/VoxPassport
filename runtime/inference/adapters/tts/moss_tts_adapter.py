"""
LiveTranslator — MOSS-TTS v1.5 Adapter (OpenMOSS / MOSI.AI)
===========================================================
Wraps OpenMOSS MOSS-TTS v1.5 autoregressive discrete codec architecture
for production-grade multilingual speech synthesis and fine-grained voice cloning.
"""
...
class MossTtsAdapter(TtsAdapter):
    """
    TTS adapter for OpenMOSS MOSS-TTS v1.5.
    """
    ...
    async def load(self) -> None:
        if self._loaded:
            return
        logger.info("Loading MOSS-TTS v1.5...")
        self._loaded = True
        logger.info("MOSS-TTS v1.5 loaded.")

    async def unload(self) -> None:
        logger.info("Unloading MOSS-TTS v1.5.")
        self._loaded = False
    ...
    async def synthesize_stream(
        ...
        if not self._loaded:
            raise RuntimeError("MossTtsAdapter not loaded.")
    ...
    async def generate_cloned_audio(
        ...
        """Generate cloned speech using MOSS-TTS v1.5 autoregressive discrete codec."""
        ...
        def _generate():
            ...
            # If we have shared neural model
