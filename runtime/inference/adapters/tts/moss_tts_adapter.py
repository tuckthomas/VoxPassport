"""Compatibility shim for the manifest-driven MOSS-TTS plugin."""
...
class MossTtsAdapter(ManifestTtsAdapter):
    ...
    def __init__(
        ...
        if shared_engine is not None:
            logger.warning("Ignoring shared_engine for manifest-driven MOSS TTS")
        catalog = TtsManifestCatalog().load()
        super().__init__(catalog.resolve(self.MODEL_ID), profiles_root=profiles_root, catalog=catalog)
        if endpoint_url:
            logger.warning(
                "MossTtsAdapter(endpoint_url=...) is deprecated; set VOXPASSPORT_MOSS_TTS_URL for the backend or VOXPASSPORT_TTS_HOST_URL for the generic host"
            )
