"""
LiveTranslator — VoxCPM 2 Adapter (OpenBMB)
===========================================
Wraps OpenBMB VoxCPM 2 tokenizer-free continuous latent diffusion architecture
for ultra-high-fidelity zero-shot voice cloning and cross-lingual synthesis.
"""
...
class VoxCpmTtsAdapter(TtsAdapter):
    """
    TTS adapter for OpenBMB VoxCPM 2.
    """
    ...
    async def load(self) -> None:
        if self._loaded:
            return
        logger.info("Loading VoxCPM 2...")
        self._loaded = True
        logger.info("VoxCPM 2 loaded.")

    async def unload(self) -> None:
        logger.info("Unloading VoxCPM 2.")
        self._loaded = False
    ...
    async def synthesize_stream(
        ...
        if not self._loaded:
            raise RuntimeError("VoxCpmTtsAdapter not loaded.")
    ...
    async def generate_cloned_audio(
        ...
        """Generate cloned speech using VoxCPM 2 continuous latent diffusion."""
