"""Outbound real-time speech translation pipeline."""
...
class OutboundTranslationPipeline:
    def __init__(
        ...
        target_language: LanguageCode = LanguageCode.RO,
    ...
    async def start(self) -> None:
        ...
        logger.info("Outbound pipeline active: %s -> %s", self.source_language.value, self.target_language.value)
        ...
        while self._is_running:
            ...
            if isinstance(self.vad_adapter, SileroVadAdapter) and self._vad_state:
                try:
                    ...
                    logger.debug("VAD frame processing failed", exc_info=True)
                ...
                if ve.event_type == VadEventType.SPEECH_START:
                    ...
                    if self._current_asr_stream and hasattr(self.asr_adapter, "endpoint"):
                        try:
                            ...
                            logger.exception("ASR endpoint decode failed")
            ...
            if self._current_asr_stream:
                try:
                    ...
                    logger.debug("ASR audio push failed", exc_info=True)
    ...
    async def _translate_and_synthesize(self, phrase: CommittedPhrase) -> None:
        try:
            ...
            logger.exception("Outbound translate/synthesize failed")
